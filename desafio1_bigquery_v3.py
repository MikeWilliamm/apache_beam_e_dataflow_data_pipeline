...
#Executando options no momento da execução no terminal. Comando para subir script ao GCP como teplate.
#python desafio1_bigquery_v3.py \ --runner=DataflowRunner \ --project=teste-dataflow-410816 \ --region=southamerica-east1 \ --staging_location=gs://teste_apache_beam_dataflow/temp \ --temp_location=gs://teste_apache_beam_dataflow/temp \ --template_location=gs://teste_apache_beam_dataflow/template/desafio1_bigquery_v3 \ --requirements_file=C:\Users\mikew\Desktop\data_flow\desafio1\requirements.txt
    ...
    # with beam.Pipeline(options=PipelineOptions()) as p:
        for file in list_files:
            content = (
                ...
                #Forma v1 para leitura de dados:
            ...
            create_dict = (
                ...
                #Se sua função não precisa de argumentos adicionais, usar a chamada mais concisa.
                | f"Transforma dados em dicionário {file}" >> beam.Map(generate_dict)
            )


            tabela = 'teste-dataflow-410816:teste_geral.dataflow_metadata'
            table_schema = 'store_reference:STRING, channel_id:INTEGER, output_channel_id:INTEGER, shared_integration_channels:STRING, origin:STRING, revision:STRING, version:STRING, fullcharge:BOOL, file:STRING, sent_rows_count:INTEGER, type:STRING, register_date:TIMESTAMP, ts:TIMESTAMP, creation_date:DATE'
            create_dict | f"Save data Big Query {file}" >> beam.io.WriteToBigQuery(
                                                                        tabela,
                                                                        schema=table_schema,
                                                                        write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND,
                                                                        create_disposition=beam.io.BigQueryDisposition.CREATE_IF_NEEDED,
                                                                        custom_gcs_temp_location = 'gs://teste_apache_beam_dataflow/temp'
                                                                        )
